...
export default function Sidebar({ collapsed = false, onToggle }: SidebarProps) {
	...
	return (
		...
		>
			...
			{/* New Project button */}
			<button
				className={`w-full bg-linear-to-br from-electric-blue to-primary-container text-white rounded-md font-semibold text-sm flex items-center justify-center gap-2 mb-6 shadow-[0_4px_15px_rgba(0,123,255,0.2)] active:scale-95 cursor-pointer ${
				...
				<a
					className={`flex items-center rounded-sm text-sm text-on-surface-variant cursor-pointer transition-all duration-150 whitespace-nowrap hover:text-on-surface hover:bg-surface-glass ${
						collapsed
							...
							: "gap-3 py-2.5 px-4 justify-between"
					}`}
				>
					...
					{!collapsed && (
						<span className="w-2 h-2 bg-electric-blue rounded-full shrink-0" />
					)}
				</a>
