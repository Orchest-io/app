...
export default function AiDescriptionGenerator({
  ...
  const handleGenerate = async () => {
    if (!context.trim()) {
      toast.warning(`Please enter a ${type} title or some context first to guide the AI.`);
      return;
    }
    ...
    try {
      ...
      if (status === 403 && data?.code === 'AI_LIMIT_REACHED') {
        ...
        toast.info('✨ AI Description Generator is coming soon! (Quota checked successfully)');
      } else {
