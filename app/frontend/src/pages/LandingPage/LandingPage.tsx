...
function TypingWord() {
	...
	const WORDS = [
		t("landing.typingWord1"),
		t("landing.typingWord2"),
		t("landing.typingWord3"),
		t("landing.typingWord4"),
	];
	...
	return (
		<span className="text-transparent bg-clip-text bg-linear-to-r from-electric-blue to-peri-purple">
			{displayed}
...
export default function LandingPage() {
	...
	const FEATURES = [
		{
			icon: "neurology",
			color: "electric-blue",
			bg: "bg-electric-blue/10 group-hover:bg-electric-blue/20",
			title: t("landing.selfHealing"),
			desc: t("landing.selfHealingDesc"),
		},
		{
			icon: "smart_toy",
			color: "peri-purple",
			bg: "bg-peri-purple/10 group-hover:bg-peri-purple/20",
			title: t("landing.contextualBriefings"),
			desc: t("landing.contextualBriefingsDesc"),
		},
		{
			icon: "verified_user",
			color: "on-surface",
			bg: "bg-on-surface/10 group-hover:bg-on-surface/20",
			title: t("landing.riskPrediction"),
			desc: t("landing.riskPredictionDesc"),
		},
	];
	...
	const SPLIT_FEATURES = [
		{
			title: t("landing.dynamicResourceMapping"),
			desc: t("landing.dynamicResourceMappingDesc"),
		},
		{
			title: t("landing.universalConnector"),
			desc: t("landing.universalConnectorDesc"),
		},
		{
			title: t("landing.enterpriseSecurity"),
			desc: t("landing.enterpriseSecurityDesc"),
		},
	];
		...
		<div className="bg-mesh min-h-screen text-on-surface font-body selection:bg-electric-blue/30 selection:text-white">
			...
			<nav className="fixed top-0 left-0 right-0 z-50 flex justify-between items-center px-6 md:px-10 h-20 backdrop-blur-md border-b border-border-low bg-bg-deep/75">
				<div
					...
					onClick={() => navigate("/")}
				>
					<img src="/orkest-logo.png" alt="Orkest" className="h-16 w-auto" />
				</div>
			...
			<main className="relative pt-32 pb-12">
				...
				<section
					id="hero"
					className="max-w-7xl mx-auto px-6 md:px-8 text-center mb-24 relative"
				>
					...
					<Reveal>
						<div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full glass-card border border-peri-purple/20 mb-8 hover:border-peri-purple/35 transition-colors duration-300">
							<span
								className="material-symbols-outlined text-peri-purple text-sm animate-pulse"
					...
					<Reveal delay={100}>
						<h1 className="font-heading text-4xl md:text-6xl font-extrabold text-on-surface mb-6 leading-tight max-w-4xl mx-auto tracking-tight min-h-[4rem] md:min-h-28">
					...
					<Reveal delay={300}>
						<div className="flex flex-col sm:flex-row items-center justify-center gap-4 mb-8 relative z-10">
							<button
								className="px-8 py-3.5 bg-linear-to-r from-electric-blue to-blue-600 rounded-full font-heading text-sm text-white font-bold electric-glow transition-transform hover:scale-105 active:scale-95 cursor-pointer"
								onClick={() => navigate(startPath)}
							>
								{isLoggedIn
					...
					<Reveal delay={400}>
						<p className="text-[11px] text-on-surface-variant mb-16 tracking-wide">
							{t("landing.trustedBy")}{" "}
							<span className="text-on-surface font-semibold">2,400+</span>{" "}
							{t("landing.teamsWorldwide")}
						</p>
					</Reveal>
					...
					<Reveal delay={200} direction="none">
						<div className="relative max-w-5xl mx-auto">
							<div className="glass-card rounded-[24px] sm:rounded-4xl overflow-hidden border border-white/10 shadow-2xl relative z-10 bg-surface-container-lowest">
								<img
								...
								<div className="absolute top-8 right-8 w-64 glass-card p-5 rounded-2xl border border-white/20 peri-glow hidden lg:block text-left z-20 animate-[float_4s_ease-in-out_infinite]">
									<div className="flex items-center gap-2 mb-3">
										<span
											className="material-symbols-outlined text-peri-purple text-lg"
											style={{ fontVariationSettings: "'FILL' 1" }}
										>
											insights
										</span>
									...
									<div className="mt-4 pt-3 border-t border-white/5 flex gap-2">
										<button
											...
											onClick={() =>
												toast.success(t("landing.insightAccepted"))
											}
										>
											{t("landing.accept")}
										</button>
										<button
											...
											onClick={() => toast.info(t("landing.insightDismissed"))}
										>
											{t("landing.ignore")}
										</button>
									</div>
										...
										<div className="flex items-center justify-between">
											<div className="flex -space-x-2.5">
												{["SC", "MO", "+4"].map((a, i) => (
													<div
														key={i}
														className="w-8 h-8 rounded-full bg-linear-to-br from-blue-500 to-indigo-500 border-2 border-bg-deep flex items-center justify-center text-white font-heading text-[9px] font-bold"
													>
														{a}
													</div>
												))}
							...
							<div className="flex flex-col items-center gap-2 mt-12 text-on-surface-variant animate-bounce">
								<span className="text-[11px] font-heading uppercase tracking-widest">
				...
				<section
					id="product"
					className="max-w-7xl mx-auto px-6 md:px-8 mb-24 scroll-mt-24"
				>
					<Reveal>
					...
					<div className="grid grid-cols-1 md:grid-cols-3 gap-6">
						{FEATURES.map(({ icon, color, bg, title, desc }, i) => (
							<Reveal key={title} delay={i * 120} direction="up">
								<Card
									hoverable
									className={`p-8 group border border-border-low transition-all duration-300 hover:-translate-y-2 hover:border-${color}/30 h-full`}
								>
									<div
										className={`w-12 h-12 rounded-xl ${bg} flex items-center justify-center mb-6 text-${color} transition-colors`}
									>
				...
				<section className="max-w-7xl mx-auto px-6 md:px-8 mb-24">
					<div className="glass-card rounded-4xl md:rounded-[40px] p-8 md:p-14 overflow-hidden relative border border-border-low">
						...
						<div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
							<Reveal direction="left">
								<div>
									<h2 className="font-heading text-2xl md:text-4xl font-extrabold text-on-surface mb-8 leading-tight tracking-tight">
										...
										<span className="text-peri-purple">
											{t("landing.predictiveIntelligence")}
										</span>
									</h2>
									...
									<ul className="space-y-6">
										{SPLIT_FEATURES.map(({ title, desc }) => (
											<li key={title} className="flex items-start gap-4 group">
												<span className="material-symbols-outlined text-electric-blue mt-0.5 text-xl transition-transform group-hover:scale-110">
													check_circle
												</span>
												<div>
							...
							<Reveal direction="right">
								<div className="relative">
									<div className="aspect-square rounded-[24px] md:rounded-4xl overflow-hidden glass-card border border-white/5 shadow-2xl relative">
										<img
											alt="AI Network"
										...
										<div className="absolute inset-0 flex items-center justify-center bg-black/10">
											<div className="p-8 rounded-full bg-surface-container-lowest/80 border border-peri-purple/30 shadow-2xl animate-pulse">
												<span
													className="material-symbols-outlined text-peri-purple text-5xl"
													style={{ fontVariationSettings: "'FILL' 1" }}
												>
													hub
												</span>
						...
						{[
							{
								...
								value: (
									<>
										<Counter to={2400} suffix="+" duration={2000} />
									</>
								),
								label: t("landing.teamsLabel"),
							},
							{
								icon: "task_alt",
								...
								value: (
									<>
										<Counter to={18} suffix="M+" duration={1800} />
									</>
								),
								label: t("landing.tasksOrchestrated"),
							},
							{
								icon: "speed",
								...
								value: (
									<>
										<Counter to={60} suffix="%" duration={1500} />
									</>
								),
								label: t("landing.avgTimeSaved"),
							},
							...
							<Reveal key={label} direction="up" delay={i * 100}>
								<Card className="p-8 flex items-center gap-6 border border-border-low">
									...
									<div>
										...
										<p className="font-heading text-[10px] text-on-surface-variant uppercase tracking-wider font-bold mt-1">
											{label}
										</p>
									</div>
								</Card>
							...
							<Reveal key={name} delay={i * 120} direction="up">
								<Card className="p-8 flex flex-col gap-5 border border-border-low hover:border-white/15 transition-colors h-full">
									...
									<div className="flex items-center gap-3 pt-4 border-t border-border-low">
										<div
											className={`w-10 h-10 rounded-full bg-linear-to-br ${color} flex items-center justify-center text-white font-heading font-bold text-xs shrink-0`}
										>
											{avatar}
										</div>
										<div>
				...
				>
					...
					<div className="grid grid-cols-1 md:grid-cols-3 gap-6 items-stretch">
						{PLANS.map(
							(
								{ name, price, desc, color, badge, features, cta, highlight },
								i,
							) => (
								<Reveal key={name} delay={i * 120} direction="up">
									<div
										className={`relative flex flex-col rounded-2xl border p-8 h-full transition-all duration-300 hover:-translate-y-1 ${color} ${highlight ? "bg-electric-blue/5 shadow-[0_0_40px_rgba(0,123,255,0.12)]" : "glass-card"}`}
									>
										{badge && (
										...
										<div className="mb-8">
											{price !== null ? (
												<div className="flex items-end gap-1">
													<span className="font-heading text-5xl font-extrabold text-on-surface">
														${price}
													</span>
													<span className="font-body text-sm text-on-surface-variant mb-1">
														{t("landing.perMonth")}
													</span>
												</div>
											) : (
												<span className="font-heading text-4xl font-extrabold text-on-surface">
													{t("landing.custom")}
												</span>
											)}
										</div>
										...
										<ul className="space-y-3 mb-8 flex-1">
											{features.map((f) => (
												...
												>
													<span
														className="material-symbols-outlined text-electric-blue text-[16px]"
														style={{ fontVariationSettings: "'FILL' 1" }}
													>
														check_circle
													</span>
										...
										<button
											...
											onClick={() =>
												name === t("landing.enterprise")
													? toast.info(t("landing.contactSalesMsg"))
													: navigate(startPath)
											}
										>
											{cta}
				...
				<section className="max-w-4xl mx-auto px-6 md:px-8 mb-24 text-center">
					<Reveal direction="none">
						<div className="glass-card rounded-4xl p-12 md:p-16 border border-electric-blue/20 relative overflow-hidden">
							<div className="absolute inset-0 bg-[radial-gradient(circle_at_50%_0%,rgba(0,123,255,0.12),transparent_60%)] pointer-events-none" />
							<span
								className="material-symbols-outlined text-electric-blue text-5xl mb-6 block"
								style={{ fontVariationSettings: "'FILL' 1" }}
							>
								rocket_launch
							</span>
							...
							<div className="flex flex-col sm:flex-row items-center justify-center gap-4">
								<button
									className="px-10 py-4 bg-linear-to-r from-electric-blue to-blue-600 rounded-full font-heading text-sm text-white font-bold electric-glow transition-transform hover:scale-105 active:scale-95 cursor-pointer"
									onClick={() => navigate(startPath)}
								>
									{isLoggedIn
										? t("landing.goToDashboard")
						...
						<div>
							...
							<ul className="space-y-4 font-body text-xs text-on-surface-variant">
								{[
									...
									<li key={l}>
										<a className="hover:text-electric-blue transition-colors cursor-pointer">
											{l}
										</a>
									</li>
								))}
