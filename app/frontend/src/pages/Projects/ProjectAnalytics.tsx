			...
			<div className="p-8 max-w-7xl mx-auto space-y-6">
				...
				<div className="grid grid-cols-1 md:grid-cols-3 gap-6">
					{[1, 2, 3].map((i) => (
						<div
							key={i}
							className="h-36 bg-white/5 animate-pulse rounded-2xl"
						/>
					))}
				</div>
	...
	return (
		<div className="min-h-screen bg-surface p-6 lg:p-8">
			<div className="max-w-7xl mx-auto space-y-8">
				...
				{isPM ? <PMView data={data} /> : <MemberView data={data} />}
			</div>
		</div>
	);
}
							...
							>
								<div className="flex items-center gap-3">
									{member.avatarUrl ? (
										...
										<div className="w-10 h-10 rounded-full bg-linear-to-br from-electric-blue/40 to-purple-500/40 flex items-center justify-center font-bold text-white border-2 border-surface text-sm">
...
function MemberView({ data }: { data: ContextualAnalyticsDto }) {
	...
	const accuracyRatio =
		personalSummary.myEstimatedHours > 0
			? Math.round(
					(personalSummary.myActualHours / personalSummary.myEstimatedHours) *
						100,
				)
			: 0;
