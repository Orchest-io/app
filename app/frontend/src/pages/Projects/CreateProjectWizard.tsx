...
export default function CreateProjectWizard() {
	...
	const [editingAiMilestoneIndex, setEditingAiMilestoneIndex] = useState<
		number | null
	>(null);
	const [editingAiTaskIndices, setEditingAiTaskIndices] = useState<{
		milestoneIdx: number;
		taskIdx: number;
	} | null>(null);
	const [aiMilestoneForm, setAiMilestoneForm] = useState({
		title: "",
		description: "",
		estimatedWeeks: 1,
	});
	...
	const handleDeleteMilestone = (milestoneIdx: number) => {
		...
		const updatedMilestones = editablePlan.milestones.filter(
			(_: any, i: number) => i !== milestoneIdx,
		);
		setEditablePlan({ ...editablePlan, milestones: updatedMilestones });
	...
	const handleDeleteTask = (milestoneIdx: number, taskIdx: number) => {
		...
		const updatedMilestones = [...editablePlan.milestones];
		updatedMilestones[milestoneIdx].tasks = updatedMilestones[
			milestoneIdx
		].tasks.filter((_: any, i: number) => i !== taskIdx);
		setEditablePlan({ ...editablePlan, milestones: updatedMilestones });
			...
			<div className="grid grid-cols-1 md:grid-cols-2 gap-6">
				...
				>
					<div className="flex flex-col items-center text-center">
						<div className="w-16 h-16 rounded-full bg-linear-to-br from-purple-500/20 to-electric-blue/20 flex items-center justify-center mb-4">
				...
				>
					<div className="flex flex-col items-center text-center">
						<div className="w-16 h-16 rounded-full bg-linear-to-br from-electric-blue/20 to-emerald-400/20 flex items-center justify-center mb-4">
	...
	const renderTypeSelection = () => (
		<div className="max-w-2xl mx-auto">
			...
			<div className="grid grid-cols-1 md:grid-cols-2 gap-6">
				...
				>
					<div className="flex flex-col items-center text-center">
						<div className="w-16 h-16 rounded-full bg-linear-to-br from-amber-400/20 to-orange-400/20 flex items-center justify-center mb-4">
	...
	const renderBasicInfoForm = () => {
		...
		const handleCreateNow = (e: React.FormEvent) => {
			...
			createProjectMutation.mutate(projectData as any, {
				...
				onError: (error: any) => {
					const errorMsg =
						error?.response?.data?.message || t("wizard.failedCreateProject");
								...
								<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
									<Select
										...
										options={[
											...
											{
												value: "completed",
												label: t("wizard.statusCompleted"),
											},
						...
						<div className="mb-8 border-t border-white/5 pt-8">
							...
							<div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4">
								{storyPointConfigs.map((config, index) => (
									<div
										key={config.storyPointValue}
										className="bg-white/5 p-3 rounded-lg border border-white/10 flex flex-col items-center"
									>
										<span className="text-xs font-bold text-electric-blue mb-2">
											{config.storyPointValue} SP
										</span>
										...
										<div className="flex items-center gap-1.5 w-full">
											<input
												...
												onChange={(e) => {
													...
													newConfigs[index].hoursEquivalent =
				...
				if (teamMembers.length > 0) {
					...
					for (const member of teamMembers) {
						try {
							...
							toast.success(
								t("wizard.memberAddedCount", {
									name: member.email,
									defaultValue: `Added ${member.email}`,
								}),
							);
						} catch (err: any) {
							const errorMsg =
							...
							if (err?.response?.status === 404) {
								...
								toast.error(
									`${member.email} ${t("wizard.memberAlreadyAdded")}`,
								);
							} else {
			...
			<div className="max-w-5xl mx-auto">
						...
						<div>
							...
							<div className="flex gap-4 text-xs text-on-surface-variant">
								...
								<span className="flex items-center gap-1">
									...
									{t("wizard.priority")}:{" "}
									{t(
										`wizard.priority${formData.priority.charAt(0).toUpperCase() + formData.priority.slice(1)}`,
									)}
								</span>
								{projectType && (
									<span className="flex items-center gap-1">
										...
										<div className="flex justify-between items-start">
											<div className="flex-1">
												<div className="flex items-center gap-2 mb-1">
													...
													>
														{t(
															`wizard.priority${task.priority.charAt(0).toUpperCase() + task.priority.slice(1)}`,
														)}
													</span>
													<span className="text-[10px] uppercase font-bold tracking-wider px-2 py-0.5 rounded border text-electric-blue bg-electric-blue/10 border-electric-blue/20">
														{task.status === "todo"
															...
															: task.status === "in-progress"
																? t("wizard.statusInProgress")
																: task.status === "review"
																	? t("wizard.statusReview")
																	: task.status === "done"
																		? t("wizard.statusDone")
																		: task.status === "backlog"
																			? t("wizard.statusBacklog")
																			: task.status}
													</span>
												</div>
												<p className="text-xs text-on-surface-variant mt-1 line-clamp-2">
				...
				<Card variant="glass" padding="lg">
					...
					<div className="flex gap-3 justify-center">
						...
						<Button onClick={handleFinalCreate} disabled={submitting}>
							{submitting
								? t("wizard.creatingProject")
	...
	const renderAiInputForm = () => {
		const handleStartGeneration = async (e: React.FormEvent) => {
			...
			try {
				...
				if (status === 403 && data?.code === "AI_LIMIT_REACHED") {
					...
					toast.error(data?.message || t("wizard.generationFailed"));
				}
			}
		};
		...
		const handleAddTeamMember = (userId: string) => {
			...
			toast.success(
				t("wizard.memberAddedCount", {
					name: user.fullName || user.email,
					defaultValue: `Added ${user.fullName || user.email}`,
				}),
			);
		};
						...
						<div className="flex flex-col gap-4">
							...
							<Select
								...
								onChange={(e) =>
									setAiFormData({
										...aiFormData,
										timelinePreference: e.target.value as any,
									})
								}
								options={[
								...
								<div className="flex items-center justify-between mb-3">
									...
									<span className="text-xs text-on-surface-variant">
										{t("wizard.selectedCount", {
											count: aiFormData.teamMembers.length,
										})}
									</span>
								</div>
														...
														<div className="flex flex-col gap-2">
															...
															<Select
																...
																options={[
																	{
																		value: "full-time",
																		label: t("wizard.fullTime"),
																	},
																	{
																		value: "part-time",
																		label: t("wizard.partTime"),
																	},
																]}
															/>
														</div>
			...
			<div className="max-w-3xl mx-auto">
				<div className="text-center mb-8">
					...
					<p className="text-sm text-on-surface-variant">
						{aiStage
							? t(
									`wizard.stage${aiStage.charAt(0).toUpperCase() + aiStage.slice(1)}`,
									{ defaultValue: aiStage },
								)
							: t("wizard.initializing")}
					</p>
				</div>
						...
						<div className="flex-1">
							...
							<div className="flex items-center gap-4 text-sm text-on-surface-variant">
								<span className="flex items-center gap-1">
									...
									{t(
										`wizard.priority${editablePlan.complexity.charAt(0).toUpperCase() + editablePlan.complexity.slice(1)}`,
									)}{" "}
									{t("wizard.complexity")}
								</span>
								<span className="flex items-center gap-1">
								...
								<span className="flex items-center gap-1">
									...
									{editablePlan.milestones.length}{" "}
									{t("projectDetails.milestonesTitle")}
								</span>
								<span className="flex items-center gap-1">
									<span className="material-symbols-outlined text-[16px]">
										task_alt
									...
									<div className="flex-1 min-w-0">
										...
										<div className="flex items-center gap-3 text-xs text-on-surface-variant">
											<span className="flex items-center gap-1">
												...
												{t("wizard.weeksCount", {
													count: milestone.estimatedWeeks,
												})}
											</span>
											<span className="flex items-center gap-1">
												<span className="material-symbols-outlined text-[14px]">
													...
													>
														<div className="flex items-start gap-2">
															<div className="w-5 h-5 rounded-full bg-electric-blue/10 flex items-center justify-center shrink-0 mt-0.5">
																<span className="text-[10px] font-bold text-electric-blue">
																	{tIdx + 1}
																</span>
															</div>
															<div className="flex-1 min-w-0">
																...
																<div className="flex flex-wrap items-center gap-x-4 gap-y-1 mt-1 text-xs text-on-surface-variant">
																	...
																	{task.type && (
																		<span className="text-[10px] uppercase tracking-wider px-1.5 py-0.5 rounded bg-surface-glass border border-border-low">
																			{task.type === "feature"
																				? t("wizard.featureType")
																				: task.type === "bug"
																					? t("wizard.bugType")
																					: task.type === "chore"
																						? t("wizard.choreType")
																						: task.type === "research"
																							? t("wizard.researchType")
																							: task.type === "improvement"
																								? t("wizard.improvementType")
																								: task.type}
																		</span>
																	)}
																</div>
				...
				<div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
					<Card className="max-w-lg w-full" padding="lg">
						...
						<form onSubmit={handleAddMilestone} className="flex flex-col gap-4">
							...
							<TextArea
								label={t("wizard.description")}
								placeholder={t("wizard.milestoneDescPlaceholder", {
									defaultValue: "Detailed description of this milestone...",
								})}
			...
			{isAiMilestoneModalOpen && (
				<div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
					<Card className="max-w-lg w-full" padding="lg">
						<div className="flex justify-between items-center mb-6">
							<h3 className="font-heading text-xl font-semibold text-on-surface">
								{editingAiMilestoneIndex !== null
									? t("wizard.editMilestone")
									: t("wizard.addMilestone")}
							</h3>
							<button
								className="text-on-surface-variant hover:text-on-surface cursor-pointer"
			...
			{isAiTaskModalOpen && (
				<div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
					<Card className="max-w-lg w-full" padding="lg">
						<div className="flex justify-between items-center mb-6">
							<h3 className="font-heading text-xl font-semibold text-on-surface">
								{editingAiTaskIndices && editingAiTaskIndices.taskIdx >= 0
									? t("wizard.editTask")
									: t("wizard.addTask")}
							</h3>
							<button
						...
						<form onSubmit={handleSaveTask} className="flex flex-col gap-4">
							<Input
								...
								onChange={(e) =>
									setAiTaskForm({ ...aiTaskForm, title: e.target.value })
								}
								required
							/>
							...
							<TextArea
								...
								placeholder={t("wizard.taskDescPlaceholder", {
									defaultValue: "Detailed description of the task...",
								})}
								value={aiTaskForm.description}
								onChange={(e) =>
									setAiTaskForm({ ...aiTaskForm, description: e.target.value })
								}
								rows={4}
								required
							/>
							...
							<div className="grid grid-cols-2 gap-4">
								<Select
									...
									onChange={(e) =>
										setAiTaskForm({ ...aiTaskForm, type: e.target.value })
									}
									options={[
										...
										{
											value: "improvement",
											label: t("wizard.improvementType"),
										},
									]}
									required
								/>
								...
								<Select
									...
									onChange={(e) =>
										setAiTaskForm({ ...aiTaskForm, priority: e.target.value })
									}
									options={[
										{ value: "low", label: t("wizard.priorityLow") },
										{ value: "medium", label: t("wizard.priorityMedium") },
							...
							<div className="grid grid-cols-2 gap-4">
								...
								<Select
									...
									options={[
										...
										{
											value: "in-progress",
											label: t("wizard.statusInProgress"),
										},
							...
							<Select
								...
								options={[
									{ value: "", label: t("wizard.selectPosition") },
									{
										...
										label: t("wizard.roleFullStack"),
									},
									{
										value: "Backend Developer",
										label: t("wizard.roleBackend"),
									},
									{
										value: "Frontend Developer",
										label: t("wizard.roleFrontend"),
									},
									{ value: "AI Specialist", label: t("wizard.roleAi") },
									{
										value: "Product Designer",
										label: t("wizard.roleDesigner"),
									},
