...
function AuthField({
	...
	return (
		<div className="flex flex-col gap-1.5">
			<div className="flex min-h-4.5 items-center justify-between gap-3">
				<label
					htmlFor={id}
...
function AuthLogo() {
	...
	return (
		<div className="flex items-center gap-4">
			<img src="/orkest-logo.png" alt="Orkest" className="h-14 w-auto" />
		</div>
	);
}
...
function MarketingScene() {
	const { t } = useTranslation();
	return (
		<section className="relative min-h-155 overflow-hidden bg-[#08101a] lg:min-h-screen">
			...
			<div className="absolute right-[16%] top-[-7%] h-160 w-160 rounded-full border border-white/8" />
			<div className="absolute right-[7%] top-[-12%] h-190 w-190 rounded-full border border-white/7" />

			<div className="absolute bottom-45 left-[18%] h-115 w-19.5 -skew-y-6 border border-cyan-300/15 bg-linear-to-b from-cyan-200/20 via-[#102637]/75 to-black/30 shadow-[0_0_42px_rgba(80,218,255,0.16)]" />
			<div className="absolute bottom-43.75 left-[26%] h-130 w-47.5 -skew-y-3 border border-cyan-300/12 bg-linear-to-br from-cyan-100/18 via-[#08131f]/95 to-black/50 shadow-[0_0_70px_rgba(0,123,255,0.18)]" />
			<div className="absolute bottom-43.75 left-[43%] h-155 w-26.25 skew-y-3 border border-cyan-300/14 bg-linear-to-b from-cyan-100/18 via-[#0d2233]/80 to-black/55 shadow-[0_0_60px_rgba(93,224,255,0.15)]" />
			<div className="absolute bottom-43.75 right-[20%] h-150 w-22 skew-y-2 border border-cyan-300/12 bg-linear-to-b from-cyan-100/16 via-[#10283b]/75 to-black/50" />

			<div className="absolute bottom-19.5 left-0 right-0 h-64 bg-[repeating-linear-gradient(0deg,rgba(75,218,255,0.13)_0px,rgba(75,218,255,0.13)_2px,transparent_2px,transparent_19px)] opacity-55 blur-[1px]" />
			<div className="absolute bottom-0 left-0 right-0 h-sidebar-w bg-linear-to-t from-black via-black/45 to-transparent" />

			<div className="absolute bottom-27.5 left-[7%] h-36 w-24 rotate-17 border border-cyan-200/20 bg-linear-to-br from-cyan-200/24 to-transparent shadow-[0_0_28px_rgba(84,222,255,0.13)]" />
			<div className="absolute bottom-23 right-[11%] h-48 w-28 rotate-17 border border-cyan-200/20 bg-linear-to-br from-cyan-200/30 to-transparent shadow-[0_0_34px_rgba(84,222,255,0.16)]" />
			<div className="absolute bottom-31.25 right-[24%] h-24 w-20 rotate-45 border border-cyan-200/16 bg-linear-to-br from-cyan-200/22 to-transparent" />

			<Card
				variant="glass"
				rounded="xl"
				padding="lg"
				className="absolute bottom-10 left-6 right-6 border-electric-blue/35 bg-[#17333b]/72 shadow-[0_34px_100px_rgba(0,0,0,0.48)] backdrop-blur-[22px] sm:left-auto sm:right-12 sm:w-140 lg:bottom-22 lg:right-[9%] xl:right-[12%]"
			>
				...
				<h2 className="max-w-117.5 font-heading text-[31px] font-extrabold leading-[1.18] tracking-[-0.04em] text-white drop-shadow-[0_2px_0_rgba(0,0,0,0.55)] md:text-[38px]">
					{t("auth.marketingTitle")}
				</h2>
				<p className="mt-6 max-w-130 text-[16px] leading-[1.55] text-[#d7dee8] md:text-[18px]">
	...
	const handleGoogleCredential = (response: { credential: string }) => {
		...
		googleMutation.mutate(
			{ email, fullName: name, avatarUrl: picture, authProviderId: sub },
			{
				...
				onError: (error) =>
					toast.error(getErrorMessage(error, t("auth.somethingWrong"))),
			},
		);
	};
	...
	return (
		<main className="min-h-screen bg-black text-on-surface">
			<div className="grid min-h-screen lg:grid-cols-[minmax(430px,45vw)_1fr]">
				<section className="relative flex min-h-screen items-center overflow-hidden bg-[#020304] px-6 py-12 sm:px-10 lg:px-16 xl:px-30">
					...
					<div className="relative z-10 mx-auto w-full max-w-105">
						...
						>
							...
							>
								<span>
									{isSubmitting ? t("auth.working") : copy.primaryAction}
								</span>
						...
						<div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
							<Button
								className="h-13.5 rounded-xl border-white/12 bg-white/[0.035]! text-white! hover:bg-white/6! disabled:pointer-events-none disabled:opacity-60"
								disabled={isSubmitting}
								type="button"
								variant="secondary"
								onClick={handleGoogleClick}
							>
								<span className="relative h-5 w-5 rounded-xs bg-[#151827]">
							...
							<Button
								className="h-13.5 rounded-xl border-white/12 bg-white/[0.035]! text-white! hover:bg-white/6!"
								type="button"
								variant="secondary"
