...
export class SubscriptionController {
  ...
  async portal(@CurrentUser() user: JwtPayload) {
    ...
    if (!dbUser.stripeCustomerId) {
      throw new BadRequestException('No Stripe customer associated with this account. Please subscribe first.');
    }
    return this.stripeService.createCustomerPortalSession(dbUser.stripeCustomerId);
  }
  ...
  async handleWebhook(
    ...
    switch (event.type) {
      case 'checkout.session.completed': {
        ...
        if (!userId) {
          console.error('[Stripe Webhook] No userId found in checkout.session.completed session');
          break;
        }
        ...
        if (stripeSubscriptionId) {
          try {
            ...
            console.error('[Stripe Webhook] Failed to retrieve subscription details', err);
          }
        }
        ...
        console.log(`[Stripe Webhook] User ${userId} upgraded to Pro. Sub: ${stripeSubscriptionId}`);
        break;
      }
      ...
      case 'customer.subscription.deleted': {
        ...
        const user = await this.usersService.findByStripeSubscriptionId(stripeSubscriptionId);
        ...
        if (user) {
          ...
          console.log(`[Stripe Webhook] User ${user.id} subscription deleted. Reverted to Free tier.`);
        } else {
          console.warn(`[Stripe Webhook] No user found for subscription ID: ${stripeSubscriptionId}`);
        }
        break;
      }
      ...
      case 'customer.subscription.updated': {
        ...
        if (user) {
          ...
          console.log(`[Stripe Webhook] User ${user.id} subscription updated. Status: ${subscription.status}`);
        } else {
          console.warn(`[Stripe Webhook] No user found for subscription ID: ${stripeSubscriptionId}`);
        }
        break;
